@if (isset($email->username) or isset($user->username))
    <div class="d-flex align-items-center">
        <div style="width: {{ $size['width'] ?? '27px' }} ; height: {{ $size['height'] ?? '27px' }};"
            class="{{ $name ?? null ? 'mr-2' : '' }} bg-{{ isset($email->color_by_user) ? $email->color_by_user : $user->color }} d-flex justify-content-center align-items-center rounded-circle"
            data-placement="top" data-toggle="tooltip"
            {{ $tooltip ?? true ? 'title=' . (isset($email->username) ? $email->username : $user->username) . '' : '' }}>
