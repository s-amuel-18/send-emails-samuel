                            ...
                            <div class="d-flex flex-wrap">
                                @foreach ($permissions as $permission)
                                    <div class="form-check m-2 ">
                                        <input id="{{ "permission_" . $permission->id }}" class="form-check-input @error('permissions') is-invalid @enderror" type="checkbox" name="permissions[]" value="{{ $permission->id }}" {{ $role->permissions->where("id", "=", $permission->id)->count() > 0 ? "checked": "" }} >
